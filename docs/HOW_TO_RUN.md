...
**IMPORTANT: Network monitoring is automatically enabled with eBPF. Only configure auditd if using auditd fallback:**
```bash
# Run with eBPF collector (primary choice - superior performance, automatically falls back to auditd if unavailable)
sudo python3 core/simple_agent.py --collector ebpf --threshold 30

# Run in headless mode (no dashboard, for automation)
sudo python3 core/simple_agent.py --collector ebpf --threshold 30 --headless

# If eBPF is unavailable, configure auditd for network monitoring (fallback only)
