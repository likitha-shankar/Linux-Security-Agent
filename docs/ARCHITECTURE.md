...
> **📚 Documentation Structure**: This file (`ARCHITECTURE.md`) provides **text-based architecture documentation** including design rationale, configuration, deployment options, and implementation details. For **visual architecture diagrams** (ASCII art diagrams showing data flow, component interactions, ML pipeline, etc.), see [ARCHITECTURE_DIAGRAMS.md](ARCHITECTURE_DIAGRAMS.md). Both files complement each other - this file explains the "why" and "how", while ARCHITECTURE_DIAGRAMS.md shows the "what" visually.
...
The Linux Security Agent follows a modular architecture with the following key components:

- **Collector Layer**: eBPF (primary) or auditd (fallback) for syscall capture
- **Processing Layer**: Event handling, process tracking, feature extraction
- **Detection Layer**: Risk scoring engine and ML-based anomaly detection
- **Output Layer**: Real-time dashboard (Rich TUI) and JSON export

For comprehensive visual diagrams showing data flow, component interactions, ML pipeline, and detailed architecture, refer to [ARCHITECTURE_DIAGRAMS.md](ARCHITECTURE_DIAGRAMS.md).
